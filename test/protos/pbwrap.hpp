...
namespace pbwrap
{
    ...
    namespace types
    {
        struct RepeatedUint32 {
            static constexpr e_field_type field_type = TYPE_UINT32;
            using type = std::vector<uint32_t>;

            static __forceinline auto get(const std::vector<std::string>& values) -> std::vector<uint32_t>
            {
                std::vector<uint32_t> result;
                for (const auto& value : values) {
                    size_t bytes_read;
                    result.push_back(utils::read_varuint32(value.data(), bytes_read));
                }
                return result;
            }
        };
